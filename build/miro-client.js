...
export class MiroClient {
    ...
    async getBoardInfo(boardId) {
        try {
            ...
            throw new Error(`Failed to fetch board info: ${error.message}`);
        }
    }
    async getBoardContent(boardId) {
        try {
            ...
            throw new Error(`Failed to extract board content: ${error.message}`);
        }
    }
    ...
    async getItemsByType(boardId, itemType) {
        try {
            const response = await this.makeRequest(`/boards/${boardId}/items?type=${itemType}`);
            return response.data || [];
        }
        catch (error) {
            throw new Error(`Failed to fetch items of type ${itemType}: ${error.message}`);
        }
    }
}
...
export class TemplateRecommendationEngine {
    ...
    analyzeContentForKeywords(content) {
    ...
    generateContextDescription(categories) {
        const contextMap = {
